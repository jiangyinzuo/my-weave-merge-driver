...
双方变化判断比较相对 base 的最终文本/分类，不观察编辑历史。所有原文比较保留格式和附着注释。当前**没有代码格式化识别、token 等价或语义等价分析**；不能把去空白后相同当作格式化，也不能据此取消冲突。
...
复制且保留源 entity 不算移动；跨文件重命名、移动同时编辑、同文件作用域/位置移动尚未识别。候选超过 10000 时明确失败，不截断为可能漏报的报告。Git 跳过 driver 的路径仍需调用方审阅 prepare 报告；协议与限制见 [global-analysis.md](global-analysis.md)。
...
`LocalAnalysis` 收集全部原因、Git 输出和区域数据后，`merge/render.rs` 只读这些结果。新增分析应在分析阶段生成事实或原因，并明确它是阻断规则还是已有原因的解释。格式化识别将来应遵循这个边界；其判定标准尚未实现。
