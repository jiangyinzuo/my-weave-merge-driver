...
driver 文本案例平铺在 [tests/fixtures/](../tests/fixtures/)，框架扫描 `*.base` 自动发现用例。新增案例无需修改 Rust 代码。
...
`FIXTURE=adjacent.ts` 会检查默认、zdiff3 及两者的详细原因模式，共四次独立运行。需要检查诊断的案例提供 `.stderr` 和 `.stderr-details`；空文件断言没有诊断。该案例在 zdiff3 模式下将未改动的 `existing()` 及共同空行留在块外，仍保留 `alpha()` / `beta()` 的冲突，退出码保持 `1`。
...
zdiff3 的失败产物名称包含 `.zdiff3`，详细模式再加 `.details`，不会互相覆盖。
...
- `tests/fixtures/`：44 组完整三方输入与预期输出；当前共 57 次 CLI 组合运行。Git 的 8×8 对照从 `git-variant-0.ts` 到 `git-variant-7.ts` 读取固定版本，只组合已有文本，不生成源码。
