...
原因 [git]：LINE_CONFLICT：Git 行级冲突
  依据 [git]：Git 行级合并返回冲突
原因 [weave]：ENTITY_CONFLICT：function 计算 需人工审核
  依据 [weave]：拒绝：modify_delete；modified in ours
原因 [analyze]：ENTITY_LAYOUT_CHANGED：实体增删或顺序变化
  依据 [analyze]：三方实体或非实体区域序列不同
原因 [analyze]：GLOBAL_MODIFY_VS_MOVE：function 计算 疑似重命名并移动（计算 → 求值），与另一侧变化需共同审核
  依据 [analyze]：RENAME_MOVE_CANDIDATE：theirs 疑似重命名并移动 function 计算 → 求值 · source.py:1 → target.py:1；源 deleted、目标 added；grammar=python、类型相同；按词边界替换各自名称后，区域文本逐 byte 相同（含附着注释）；替换次数=2/2；sources=1，destinations=1；另一侧=modified；仅为文本候选，未证明语义等价
